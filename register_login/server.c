...
void send_file_list(int client_socket);                                      // 新增：发送文件列表
void send_file_to_client(int client_socket, const char *file_name);          // 新增：发送文件内容
void add_client_to_list(int client_socket, struct sockaddr_in *client_addr); // 添加客户端到链表
void remove_client_from_list(int client_socket);                             // 从链表中移除客户端
void display_client_list();                                                  // 显示当前客户端列表
void *worker_thread(void *arg);                                              // 工作线程
void init_thread_pool();                                                     // 初始化线程池
void add_task_to_queue(int client_socket);                                   // 显示当前客户端列表
void receive_file_from_client(int client_socket, const char *file_name);

int main()
{
    init_thread_pool(); // 初始化线程池
...
// 将任务添加到任务队列
void add_task_to_queue(int client_socket)
{
...
// 添加客户端到链表
void add_client_to_list(int client_socket, struct sockaddr_in *client_addr)
{
