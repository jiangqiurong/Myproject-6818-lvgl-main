...
enum {
    LV_LABEL_LONG_WRAP,   /**< 长文本换行模式Keep the object width, wrap the too long lines and expand the object
                        height*/
    LV_LABEL_LONG_DOT,    /**< 长文本打点模式Keep the size and write dots at the end if the text is too long*/
    LV_LABEL_LONG_SCROLL, /**< 长文本滚动模式Keep the size and roll the text back and forth*/
    LV_LABEL_LONG_SCROLL_CIRCULAR, /**< 长文本循环滚动模式Keep the size and roll the text circularly*/
    LV_LABEL_LONG_CLIP,            /**<  长文本裁剪模式Keep the size and clip the text out of it*/
};
...
/**
 * 设置标签对象的文本内容Set a new text for a label. Memory will be
 * allocated to store the text by the label.
...
/**
 * 设置标签对象的长文本显示模式Set the behavior of the label with longer
 * text then the object size
 * @param obj           pointer to a label object
