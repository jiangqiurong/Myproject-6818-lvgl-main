 ...
 */
typedef enum {
    LV_EVENT_ALL = 0, // 用来注册一个回调函数以接收所有类型的事件

    /** Input device events*/
    LV_EVENT_PRESSED,  /**< 对象被按下 The object has been pressed*/
    LV_EVENT_PRESSING, /**< 对象正在被按下 The object is being pressed (called continuously while pressing)*/
    LV_EVENT_PRESS_LOST, /**< 按下状态丢失 The object is still being pressed but slid cursor/finger off of the object */
    LV_EVENT_SHORT_CLICKED, /**< 短时间点击 The object was pressed for a short period of time, then released it. Not
                               called if scrolled.*/
    LV_EVENT_LONG_PRESSED,  /**< 长时间按下 Object has been pressed for at least `long_press_time`.  Not called if
                               scrolled.*/
    LV_EVENT_LONG_PRESSED_REPEAT, /**<长按重复触发 Called after `long_press_time` in every `long_press_repeat_time` ms.
                                     Not called if scrolled.*/
    LV_EVENT_CLICKED,             /**< 点击 Called on release if not scrolled (regardless to long press)*/
    LV_EVENT_RELEASED,            /**<释放 Called in every cases when the object has been released*/
    LV_EVENT_SCROLL_BEGIN,        /**<滚动开始 Scrolling begins*/
    LV_EVENT_SCROLL_END,          /**<滚动结束 Scrolling ends*/
    LV_EVENT_SCROLL,              /**<滚动中 Scrolling*/
    LV_EVENT_GESTURE,             /**<手势检测 A gesture is detected. Get the gesture with
                                   * `lv_indev_get_gesture_dir(lv_indev_get_act());`
                                   */
    LV_EVENT_KEY, /**<按键事件 A key is sent to the object. Get the key with `lv_indev_get_key(lv_indev_get_act());`*/
    LV_EVENT_FOCUSED,   /**<获得焦点 The object is focused*/
    LV_EVENT_DEFOCUSED, /**<失去焦点 The object is defocused*/
    LV_EVENT_LEAVE,     /**<离开焦点但仍被选中 The object is defocused but still selected*/
    LV_EVENT_HIT_TEST,  /**<高级命中测试 Perform advanced hit-testing*/

    /** Drawing events*/
    LV_EVENT_COVER_CHECK, /**<覆盖检查 Check if the object fully covers an area. The event parameter is
                           *`lv_cover_check_info_t
                           *`.*/
    LV_EVENT_REFR_EXT_DRAW_SIZE, /**<获取额外绘制区域大小 Get the required extra draw area around the object (e.g. for
                                    shadow). The event parameter is `lv_coord_t *` to store the size.*/
    LV_EVENT_DRAW_MAIN_BEGIN, /**<开始主绘制阶段 Starting the main drawing phase*/
    LV_EVENT_DRAW_MAIN,       /**<执行主绘制 Perform the main drawing*/
    LV_EVENT_DRAW_MAIN_END,   /**<结束主绘制阶段 Finishing the main drawing phase*/
    LV_EVENT_DRAW_POST_BEGIN, /**<开始后绘制阶段（当所有子对象都已绘制完成） Starting the post draw phase (when all
                                 children are drawn)*/
    LV_EVENT_DRAW_POST, /**<执行后绘制 Perform the post draw phase (when all children are drawn)*/
    LV_EVENT_DRAW_POST_END, /**<结束后绘制阶段（当所有子对象都已绘制完成） Finishing the post draw phase (when all
                           children are drawn)*/
    LV_EVENT_DRAW_PART_BEGIN, /**<开始绘制一个部分 Starting to draw a part. The event parameter is `lv_obj_draw_dsc_t
                               *`. */
    LV_EVENT_DRAW_PART_END, /**<结束绘制一个部分 Finishing to draw a part. The event parameter is `lv_obj_draw_dsc_t *`.
                             */

    /** Special events*/
    LV_EVENT_VALUE_CHANGED, /**<值改变 The object's value has changed (i.e. slider moved)*/
    LV_EVENT_INSERT,  /**<插入文本 A text is inserted to the object. The event data is `char *` being inserted.*/
    LV_EVENT_REFRESH, /**<刷新 Notify the object to refresh something on it (for the user)*/
    LV_EVENT_READY,   /**<准备就绪 A process has finished*/
    LV_EVENT_CANCEL,  /**<取消 A process has been cancelled */

    /** Other events*/
    LV_EVENT_DELETE,              /**<删除 Object is being deleted*/
    LV_EVENT_CHILD_CHANGED,       /**<子对象改变 Child was removed, added, or its size, position were changed */
    LV_EVENT_CHILD_CREATED,       /**<子对象创建 Child was created, always bubbles up to all parents*/
    LV_EVENT_CHILD_DELETED,       /**<子对象删除 Child was deleted, always bubbles up to all parents*/
    LV_EVENT_SCREEN_UNLOAD_START, /**<屏幕卸载开始 A screen unload started, fired immediately when scr_load is called*/
    LV_EVENT_SCREEN_LOAD_START, /**<屏幕加载开始 A screen load started, fired when the screen change delay is expired*/
    LV_EVENT_SCREEN_LOADED,   /**<屏幕已加载 A screen was loaded*/
    LV_EVENT_SCREEN_UNLOADED, /**<屏幕已卸载 A screen was unloaded*/
    LV_EVENT_SIZE_CHANGED,    /**<大小改变 Object coordinates/size have changed*/
    LV_EVENT_STYLE_CHANGED,   /**<样式改变 Object's style has changed*/
    LV_EVENT_LAYOUT_CHANGED,  /**<布局改变 The children position has changed due to a layout recalculation*/
    LV_EVENT_GET_SELF_SIZE,   /**<获取自身大小 Get the internal size of a widget*/

    _LV_EVENT_LAST, /**表示默认事件的数量 Number of default events*/
 ...
 *   - If already set to `false` an other event wants that point non clickable. If you want to respect it leave it as
 * `false` or set `true` to overwrite it.
 ...
 * @return          LV_RES_OK: the target object was not deleted in the event; LV_RES_INV: it was deleted in the
 * event_code
 */
lv_res_t lv_obj_event_base(const lv_obj_class_t * class_p, lv_event_t * e);

/**
 *获取事件的目标对象 Get the object originally targeted by the event. It's the same even if the event is bubbled.
 * @param e     pointer to the event descriptor
...
/**
 * 获取事件代码，并且去除可能存在的 LV_EVENT_PREPROCESS 标志位。Get the event code of an event
 * @param e     pointer to the event descriptor
...
/**
 * 获取用户自定义数据指针Get the user_data passed when the event was registered on the object
 * @param e     pointer to the event descriptor
...
/**
 * 向对象添加事件回调函数Add an event handler function for an object.
 * Used by the user to react on event which happens with the object.
 * An object can have multiple event handler. They will be called in the same order as they were added.
 * @param obj       pointer to an object
 * @param filter    and event code (e.g. `LV_EVENT_CLICKED`) on which the event should be called. `LV_EVENT_ALL` can be
 * sued the receive all the events.
...
/**
 * Get a pointer to an `lv_point_t` variable in which the self size should be saved (width in `point->x` and height
 * `point->y`). Can be used in `LV_EVENT_GET_SELF_SIZE`
 * @param e     pointer to an event
...
/**
 * Get a pointer to an `lv_hit_test_info_t` variable in which the hit test result should be saved. Can be used in
 * `LV_EVENT_HIT_TEST`
 * @param e     pointer to an event
