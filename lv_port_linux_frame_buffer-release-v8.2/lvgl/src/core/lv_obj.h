...
enum {
    ...
    LV_STATE_CHECKED   = 0x0001, // 表示元素处于被选中状态
    LV_STATE_FOCUSED   = 0x0002, // 表示元素获得焦点
    LV_STATE_FOCUS_KEY = 0x0004, // 表示元素通过键盘焦点键被聚焦
    LV_STATE_EDITED    = 0x0008, // 表示元素正在被编辑
    LV_STATE_HOVERED   = 0x0010, // 表示鼠标指针悬停在元素上
    LV_STATE_PRESSED   = 0x0020, // 表示元素被按下
    LV_STATE_SCROLLED  = 0x0040, // 表示元素处于滚动状态
    LV_STATE_DISABLED  = 0x0080, // 表示元素被禁用
 ...
 */
enum {
    LV_OBJ_FLAG_HIDDEN = (1L << 0), /**<使对象隐藏 Make the object hidden. (Like it wasn't there at all)*/
    LV_OBJ_FLAG_CLICKABLE = (1L << 1), /**<使对象可被输入设备点击 Make the object clickable by the input devices*/
    LV_OBJ_FLAG_CLICK_FOCUSABLE =
        (1L << 2), /**<当对象被点击时，添加聚焦状态 Add focused state to the object when clicked*/
    LV_OBJ_FLAG_CHECKABLE =
        (1L << 3), /**<当对象被点击时，切换选中状态 Toggle checked state when the object is clicked*/
    LV_OBJ_FLAG_SCROLLABLE = (1L << 4), /**<使对象可滚动 Make the object scrollable*/
    LV_OBJ_FLAG_SCROLL_ELASTIC =
        (1L << 5), /**<允许在对象内部滚动，但速度较慢 Allow scrolling inside but with slower speed*/
    LV_OBJ_FLAG_SCROLL_MOMENTUM =
        (1L << 6), /**<使对象在被 “抛掷” 时滚动得更远 Make the object scroll further when "thrown"*/
    LV_OBJ_FLAG_SCROLL_ONE = (1L << 7), /**<只允许滚动一个可捕捉的子对象 Allow scrolling only one snappable children*/
    LV_OBJ_FLAG_SCROLL_CHAIN_HOR =
        (1L << 8), /**<允许将水平滚动传播到父对象 Allow propagating the horizontal scroll to a parent*/
    LV_OBJ_FLAG_SCROLL_CHAIN_VER =
        (1L << 9), /**<允许将垂直滚动传播到父对象 Allow propagating the vertical scroll to a parent*/
    LV_OBJ_FLAG_SCROLL_CHAIN =
        (LV_OBJ_FLAG_SCROLL_CHAIN_HOR | LV_OBJ_FLAG_SCROLL_CHAIN_VER), /**< 同时包含水平和垂直滚动传播到父对象的标志*/
    LV_OBJ_FLAG_SCROLL_ON_FOCUS = (1L << 10), /**<当对象获得焦点时，自动滚动以使其可见 Automatically scroll object
                                                 to make it visible when focused*/
    LV_OBJ_FLAG_SCROLL_WITH_ARROW =
        (1L << 11), /**<允许使用箭头键滚动获得焦点的对象 Allow scrolling the focused object with arrow keys*/
    LV_OBJ_FLAG_SNAPPABLE = (1L << 12), /**<如果父对象启用了滚动捕捉，该对象可以被捕捉 If scroll snap is enabled on the
                                           parent it can snap to this object*/
    LV_OBJ_FLAG_PRESS_LOCK = (1L << 13), /**<即使按下的位置从对象上滑开，也保持对象被按下状态 Keep the object pressed
                                            even if the press slid from the object*/
    LV_OBJ_FLAG_EVENT_BUBBLE   = (1L << 14), /**<将事件传播到父对象 Propagate the events to the parent too*/
    LV_OBJ_FLAG_GESTURE_BUBBLE = (1L << 15), /**<将手势传播到父对象 Propagate the gestures to the parent*/
    LV_OBJ_FLAG_ADV_HITTEST = (1L << 16), /**<允许进行更准确的命中（点击）测试 Allow performing more accurate hit
                                             (click) test. E.g. consider rounded corners.*/
    LV_OBJ_FLAG_IGNORE_LAYOUT = (1L << 17), /**<使对象不受布局定位影响 Make the object position-able by the layouts*/
    LV_OBJ_FLAG_FLOATING = (1L << 18), /**<当父对象滚动时，不滚动该对象，并且忽略布局 Do not scroll the object when the
                                          parent scrolls and ignore layout*/
    LV_OBJ_FLAG_OVERFLOW_VISIBLE =
        (1L << 19), /**<不将子对象的内容裁剪到父对象的边界 Do not clip the children's content to the parent's boundary*/
...
/**
 * 创建画布 Create a base object (a rectangle)
...
/**
 * 控制对象的各种行为和属性Set one or more flags
...
/**
 * 获取对象的状态Get the state of an object
 * @param obj   pointer to an object
