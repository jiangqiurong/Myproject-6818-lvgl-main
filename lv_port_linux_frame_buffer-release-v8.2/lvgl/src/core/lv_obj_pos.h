...
/**
 * 设置对象的位置Set the position of an object relative to the set alignment.
...
/**
 * 设置对象的大小Set the size of an object.
 * @param obj       pointer to an object
 ...
 *                  LV_SIZE_PCT(x)     to set size in percentage of the parent's content area size (the size without
 * paddings). x should be in [0..1000]% range
 */
...
/**
 * 设置对象的宽度Set the width of an object
 * @param obj       pointer to an object
 * @param w         the new width
 * @note            possible values are:
 *                  pixel               simple set the size accordingly
 *                  LV_SIZE_CONTENT     set the size to involve all children in the given direction
 *                  lv_pct(x)           to set size in percentage of the parent's content area size (the size without
 * paddings). x should be in [0..1000]% range
 */
...
/**
 * 设置对象的对齐方式和位置偏移Change the alignment of an object and set new coordinates.
 * Equivalent to:
...
/**
 * 对象在其父容器中居中显示Align an object to the center on its parent.
 * @param obj       pointer to an object to align
 ...
 * @note            The position of the object is recalculated only on the next redraw. To force coordinate
 * recalculation call `lv_obj_update_layout(obj)`.
 ...
 * @note            The position of the object is recalculated only on the next redraw. To force coordinate
 * recalculation call `lv_obj_update_layout(obj)`.
 * @note            Zero return value means the object is on the bottom padding of the parent, and not on the bottom
 * edge.
 * @note            Scrolling of the parent doesn't change the returned value.
 * @note            The returned value is always the distance from the parent even if `obj` is positioned by a layout.
 */
