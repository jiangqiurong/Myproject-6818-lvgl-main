...
enum {
    ...
    LV_OPA_0      = 0,   // 完全透明
    LV_OPA_10     = 25,  // 不透明度为 10%
    LV_OPA_20     = 51,  // 不透明度为 20%
    LV_OPA_30     = 76,  // 不透明度为 30%
    LV_OPA_40     = 102, // 不透明度为 40%
    LV_OPA_50     = 127, // 不透明度为 50%
    LV_OPA_60     = 153, // 不透明度为 60%
    LV_OPA_70     = 178, // 不透明度为 70%
    LV_OPA_80     = 204, // 不透明度为 80%
    LV_OPA_90     = 229, // 不透明度为 90%
...
#define LV_COLOR_MAKE8(r8, g8, b8)                                                                                     \
    {                                                                                                                  \
        {                                                                                                              \
            (uint8_t)((b8 >> 6) & 0x3U), (uint8_t)((g8 >> 5) & 0x7U), (uint8_t)((r8 >> 5) & 0x7U)                      \
        }                                                                                                              \
    }
...
#define LV_COLOR_MAKE16(r8, g8, b8)                                                                                    \
    {                                                                                                                  \
        {                                                                                                              \
            (uint8_t)((b8 >> 3) & 0x1FU), (uint8_t)((g8 >> 2) & 0x3FU), (uint8_t)((r8 >> 3) & 0x1FU)                   \
        }                                                                                                              \
    }
#else
#define _LV_COLOR_ZERO_INITIALIZER16                                                                                   \
...
#define LV_COLOR_MAKE16(r8, g8, b8)                                                                                    \
    {                                                                                                                  \
        {                                                                                                              \
            (uint8_t)((g8 >> 5) & 0x7U), (uint8_t)((r8 >> 3) & 0x1FU), (uint8_t)((b8 >> 3) & 0x1FU),                   \
                (uint8_t)((g8 >> 2) & 0x7U)                                                                            \
        }                                                                                                              \
    }
#endif
...
LV_ATTRIBUTE_FAST_MEM static inline lv_color_t lv_color_mix(lv_color_t c1, lv_color_t c2, uint8_t mix)
{
    ...
    mix = (mix + 4) >> 3;
    uint32_t bg =
        (uint32_t)((uint32_t)c2.full | ((uint32_t)c2.full << 16)) & 0x7E0F81F; /*0b00000111111000001111100000011111*/
    uint32_t fg     = (uint32_t)((uint32_t)c1.full | ((uint32_t)c1.full << 16)) & 0x7E0F81F;
    uint32_t result = ((((fg - bg) * mix) >> 5) + bg) & 0x7E0F81F;
    ret.full        = (uint16_t)((result >> 16) | result);
#elif LV_COLOR_DEPTH != 1
    /*LV_COLOR_DEPTH == 8, 16 or 32*/
    LV_COLOR_SET_R(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_R(c1) * mix + LV_COLOR_GET_R(c2) * (255 - mix) +
                                   LV_COLOR_MIX_ROUND_OFS));
    LV_COLOR_SET_G(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_G(c1) * mix + LV_COLOR_GET_G(c2) * (255 - mix) +
                                   LV_COLOR_MIX_ROUND_OFS));
    LV_COLOR_SET_B(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_B(c1) * mix + LV_COLOR_GET_B(c2) * (255 - mix) +
                                   LV_COLOR_MIX_ROUND_OFS));
