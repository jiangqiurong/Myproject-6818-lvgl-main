...
/** Alignments*/
enum {
    LV_ALIGN_DEFAULT = 0,  // 默认对齐方式，具体行为可能取决于其他设置或上下文
    LV_ALIGN_TOP_LEFT,     // 将元素对齐到容器的左上角
    LV_ALIGN_TOP_MID,      // 将元素水平居中对齐到容器的顶部
    LV_ALIGN_TOP_RIGHT,    // 将元素对齐到容器的右上角
    LV_ALIGN_BOTTOM_LEFT,  // 将元素对齐到容器的左下角
    LV_ALIGN_BOTTOM_MID,   // 将元素水平居中对齐到容器的底部
    LV_ALIGN_BOTTOM_RIGHT, // 将元素对齐到容器的右下角
    LV_ALIGN_LEFT_MID,     // 将元素垂直居中对齐到容器的左侧
    LV_ALIGN_RIGHT_MID,    // 将元素垂直居中对齐到容器的右侧
    LV_ALIGN_CENTER,       // 将元素在水平和垂直方向上都居中对齐到容器中

    LV_ALIGN_OUT_TOP_LEFT,     // 将元素放置在容器外部的左上角
    LV_ALIGN_OUT_TOP_MID,      // 将元素放置在容器外部的顶部中间
    LV_ALIGN_OUT_TOP_RIGHT,    // 将元素放置在容器外部的右上角
    LV_ALIGN_OUT_BOTTOM_LEFT,  // 将元素放置在容器外部的左下角
    LV_ALIGN_OUT_BOTTOM_MID,   // 将元素放置在容器外部的底部中间
    LV_ALIGN_OUT_BOTTOM_RIGHT, // 将元素放置在容器外部的右下角
    LV_ALIGN_OUT_LEFT_TOP,     // 将元素放置在容器外部的左侧顶部
    LV_ALIGN_OUT_LEFT_MID,     // 将元素放置在容器外部的左侧中间
    LV_ALIGN_OUT_LEFT_BOTTOM,  // 将元素放置在容器外部的左侧底部
    LV_ALIGN_OUT_RIGHT_TOP,    // 将元素放置在容器外部的右侧顶部
    LV_ALIGN_OUT_RIGHT_MID,    // 将元素放置在容器外部的右侧中间
    LV_ALIGN_OUT_RIGHT_BOTTOM, // 将元素放置在容器外部的右侧底部
};
